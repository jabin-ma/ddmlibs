 ...
 * <p/>
 * {@link AndroidDebugBridge} is the public API to connection to adb, while
 * {@link AdbHelper} does the low level stuff.
 * <p/>
...
final class AdbHelper {
	 ...
	 * @param adbSockAddr
	 * @param device
	 *            the device to connect to. Can be null in which case the
	 *            connection will be to the first available device.
	 * @param devicePort
	 *            the port we're opening
	...
	/**
	 * Creates and connects a new pass-through socket, from the host to a port
	 * on the device.
	 *
	 * @param adbSockAddr
	 * @param device
	 ...
	 * @param port
	 *            the port on the device. This does not need to be numeric.
	 */
	...
	/**
	 * Creates a port forwarding request to a jdwp process. This returns an
	 * array containing "####jwdp:{pid}".
	 * 
	 * @param pid
	...
	/**
	 * Create an ASCII string preceded by four hex digits. The opening "####" is
	 * the length of the rest of the string, encoded as ASCII hex (case doesn't
	 * matter).
	 */
	...
	public static byte[] formTransportRequest(String req) {
		byte[] result;
		try {
			result = req.getBytes(DEFAULT_ENCODING);
		} catch (UnsupportedEncodingException uee) {
			uee.printStackTrace(); // not expected
			return null;
		}
	 ...
	 * 
	 * @param chan
	 *            The socket channel that is connected to adb.
	 * @param readDiagString
	 *            If true, we're expecting an OKAY response to be followed by a
	 *            diagnostic string. Otherwise, we only expect the diagnostic
	 *            string to follow a FAIL.
	 * @throws TimeoutException
	 *             in case of timeout on the connection.
	 * @throws IOException
	 *             in case of I/O error on the connection.
		...
		try {
			while (readDiagString) {
				...
				try {
					...
					Log.w("ddms", "Expected digits, got '" + lenStr + "': " + lenBuf[0] + " " + lenBuf[1] + " "
							+ lenBuf[2] + " " + lenBuf[3]);
			...
			// ignore those, since it's just reading the diagnose string, the
			// response will
	...
	/**
	 * Retrieve the frame buffer from the device with the given timeout. A
	 * timeout of 0 indicates that it will wait forever.
	 *
	 * @throws TimeoutException
	 *             in case of timeout on the connection.
	 * @throws AdbCommandRejectedException
	...
	static RawImage getFrameBuffer(InetSocketAddress adbSockAddr, Device device, long timeout, TimeUnit unit)
		...
		try {
			...
			Log.d("ddms", "image params: bpp=" + imageParams.bpp + ", size=" + imageParams.size + ", width="
	...
	/**
	 * @deprecated Use
	 *             {@link #executeRemoteCommand(java.net.InetSocketAddress, String, IDevice, IShellOutputReceiver, long, java.util.concurrent.TimeUnit)}.
	 */
	@Deprecated
	static void executeRemoteCommand(InetSocketAddress adbSockAddr, String command, IDevice device,
	...
	/**
	 * Executes a shell command on the device and retrieve the output. The
	 * output is handed to <var>rcvr</var> as it arrives.
	 *
	 * @param adbSockAddr
	 *            the {@link InetSocketAddress} to adb.
	 * @param command
	 *            the shell command to execute
	 * @param device
	 *            the {@link IDevice} on which to execute the command.
	 * @param rcvr
	 *            the {@link IShellOutputReceiver} that will receives the output
	 ...
	 *            throw.
	 * @param maxTimeUnits
	 *            Units for non-zero {@code maxTimeToOutputResponse} values.
	 * @throws TimeoutException
	 *             in case of timeout on the connection when sending the
	 *             command.
	 * @throws AdbCommandRejectedException
	 *             if adb rejects the command
	 * @throws ShellCommandUnresponsiveException
	 *             in case the shell command doesn't send any output for a
	 *             period longer than <var>maxTimeToOutputResponse</var>.
	 * @throws IOException
	 *             in case of I/O error on the connection.
	 *
	 ...
	 */
	static void executeRemoteCommand(InetSocketAddress adbSockAddr, String command, IDevice device,
			IShellOutputReceiver rcvr, long maxTimeToOutputResponse, TimeUnit maxTimeUnits)
			throws TimeoutException, AdbCommandRejectedException, ShellCommandUnresponsiveException, IOException {
	...
	/**
	 * Executes a remote command on the device and retrieve the output. The
	 * output is handed to <var>rcvr</var> as it arrives. The command is execute
	 * by the remote service identified by the adbService parameter.
	 *
	 * @param adbSockAddr
	 *            the {@link InetSocketAddress} to adb.
	 * @param adbService
	 *            the {@link com.android.ddmlib.AdbHelper.AdbService} to use to
	 *            run the command.
	 * @param command
	 *            the shell command to execute
	 * @param device
	 ...
	 *            Units for non-zero {@code maxTimeToOutputResponse} values.
	 * @param is
	 *            a optional {@link InputStream} to be streamed up after
	...
	static void executeRemoteCommand(InetSocketAddress adbSockAddr, AdbService adbService, String command,
			IDevice device, IShellOutputReceiver rcvr, long maxTimeToOutputResponse, TimeUnit maxTimeUnits,
	...
	static SocketChannel createAdbSocket(InetSocketAddress adbSockAddr) throws IOException {
		SocketChannel adbChan = SocketChannel.open(adbSockAddr);
		adbChan.configureBlocking(false);
		return adbChan;
	}
...
//	}
	 ...
	 * @param localPortSpec
	 *            specification of the local port to forward, should be of
	 *            format tcp:<port number>
	 * @param remotePortSpec
	 *            specification of the remote port to forward to, one of:
	 *            tcp:<port> localabstract:<unix domain socket name>
	 *            localreserved:<unix domain socket name> localfilesystem:<unix
	 *            domain socket name> dev:<character device name> jdwp:<process
	 *            pid> (remote only)
	 * @throws TimeoutException
	 *             in case of timeout on the connection.
	 * @throws AdbCommandRejectedException
	 ...
	 *             in case of I/O error on the connection.
	 */
	public static void createForward(InetSocketAddress adbSockAddr, Device device, String localPortSpec,
			String remotePortSpec) throws TimeoutException, AdbCommandRejectedException, IOException {

		SocketChannel adbChan = null;
		try {
			adbChan = createAdbSocket(adbSockAddr);

			byte[] request = formAdbRequest(String.format("host-serial:%1$s:forward:%2$s;%3$s", //$NON-NLS-1$
	 ...
	 * @param localPortSpec
	 *            specification of the local port that was forwarded, should be
	 *            of format tcp:<port number>
	 * @param remotePortSpec
	 *            specification of the remote port forwarded to, one of:
	 *            tcp:<port> localabstract:<unix domain socket name>
	 *            localreserved:<unix domain socket name> localfilesystem:<unix
	 *            domain socket name> dev:<character device name> jdwp:<process
	 *            pid> (remote only)
	 ...
	 *             in case of I/O error on the connection.
	 */
	public static void removeForward(InetSocketAddress adbSockAddr, Device device, String localPortSpec,
			String remotePortSpec) throws TimeoutException, AdbCommandRejectedException, IOException {
		...
		try {
			...
			byte[] request = formAdbRequest(String.format("host-serial:%1$s:killforward:%2$s", //$NON-NLS-1$
	...
	/**
	 * Reads from the socket until the array is filled, or no more data is
	 * coming (because the socket closed or the timeout expired).
	 * <p/>
	 ...
	 * @param timeout
	 *            The timeout value in ms. A timeout of zero means "wait
	 *            forever".
	 */
	...
	/**
	 * Write until all data in "data" is written or the connection fails or
	 * times out.
	 * <p/>
	 * This uses the default time out value.
	 * 
	 * @param chan
	 *            the opened socket to write to.
	 * @param data
	 *            the buffer to send.
	 * @throws TimeoutException
	 *             in case of timeout on the connection.
	...
	/**
	 * Write until all data in "data" is written, the optional length is
	 * reached, the timeout expires, or the connection fails. Returns "true" if
	 ...
	 * @param timeout
	 *            The timeout value. A timeout of zero means "wait forever".
	 * @throws TimeoutException
	 *             in case of timeout on the connection.
	 * @throws IOException
	 *             in case of I/O error on the connection.
	 ...
	 *
	 * @param into
	 *            what to reboot into (recovery, bootloader). Or null to just
	 *            reboot.
	 * @throws TimeoutException
	 *             in case of timeout on the connection.
	 * @throws AdbCommandRejectedException
